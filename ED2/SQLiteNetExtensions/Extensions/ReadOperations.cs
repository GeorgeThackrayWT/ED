...
using WTSqLExt.Extensions.TextBlob;
using WTSqLExt.Exceptions;
using SQLite.Net;

namespace WTSqLExt.Extensions
{
    ...
    public static class ReadOperations
    {
        ...
        public static List<T> GetAllWithChildren<T>(this SQLiteConnection conn, 
            ...
            int? limit = null,
            int? offset = null, int recursive = 1)
            where T : class, new()
        {

        //    Debug.WriteLine("GetAllWithChildren");
            ...
            foreach (T element in list)
            {
                SetConnection(conn, element);

                //public SQLite.Net.Async.SQLiteAsyncConnection
                //Additional information: Object of type 'SQLite.Net.SQLiteConnectionWithLock' 
                //cannot be converted to type 'SQLite.Net.Async.SQLiteAsyncConnection'.

                if (recursive >0)
                    conn.GetChildren(element, recursive == 1);
            }
        ...
        private static void SetConnection<T>(SQLiteConnection conn, T element) //where T : class, new()
        {
            var c = element.GetType().GetConnection();

            c?.SetValue(element, conn, null);
        ...
        public static T GetWithChildren<T>(this SQLiteConnection conn, object pk, bool recursive = false)
			where T : class, new()
		{
            //Debug.WriteLine("GetWithChildren1");
		...
		where T : class, new()
		{
            Debug.WriteLine("FindWithChildren2");
        ...
        {
           // Debug.WriteLine("GetChildren1: " + element.GetType().Name);
        ...
        private static void GetChildrenRecursive(this SQLiteConnection conn, object element, bool onlyCascadeChildren, bool recursive, ObjectCache objectCache =  null) {
            ...
            var relProps = element.GetType().GetRelationshipProperties();

            foreach (var relationshipProperty in relProps)
            {
        ...
        private static void GetChildRecursive(this SQLiteConnection conn, object element, PropertyInfo relationshipProperty, bool recursive, ObjectCache objectCache) {
            ...
            //Debug.WriteLine("GetChildRecursive:" + relationshipAttribute.GetType().Name);
            ...
            bool recursive, ObjectCache objectCache)
        {
            ...
            //Debug.WriteLine("GetOneToOneChildren:" + recursive);
            ...
            bool recursive, ObjectCache objectCache)
        {
            ...
            foreach (T element in elements)
            {
                ...
                //Debug.WriteLine("GetManyToOneChildren: foreign key " 
                //    + entityType.Name + " " + " - " + type.Name 
                //    + " " + currentEntityForeignKeyProperty.Name + " " + foreignKeyValue);
            ...
            bool recursive, ObjectCache objectCache)
        {
           ...
           // Debug.WriteLine("GetOneToManyChildren:" + type  + " " + currentEntityPrimaryKeyProperty.Name + " " + primaryKeyValue);
        ...
        private static IEnumerable GetManyToManyChildren<T>(this SQLiteConnection conn, T element,
            ...
            bool recursive, ObjectCache objectCache)
        {
          //  Debug.WriteLine("GetManyToManyChildren:" + recursive);
